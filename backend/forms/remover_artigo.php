    ...
    if (isset($_POST['id'])) {
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                $response['message'] = 'Artigo removido com sucesso!';
            } else {
                logError("Erro ao remover artigo: " . $stmt->error);
                $response['status'] = 'error';
                $response['message'] = 'Erro ao remover Artigo: ' . $stmt->error;
            }
            $stmt->close();
        } else {
        ...
        $response['status'] = 'error';
        $response['message'] = 'Artigo não foi fornecido.';
    }
} else {
    $response['status'] = 'error';
